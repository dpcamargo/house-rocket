...
# ADD NEW FEATURES / DROP UNUSED FEATURES
...
df3 = data.sample(f_map_sampling)
...
df4 = df3[['price', 'zipcode']].groupby('zipcode').mean().reset_index().copy()
df4.columns = ['ZIP', 'PRICE']
geofile = geofile[geofile['ZIP'].isin(
    df4['ZIP'].tolist())]  # Clears ZIPs not in df4

price_density = folium.Map(
    location=[data['lat'].mean(), data['long'].mean()], default_zoom_start=15)
price_density.choropleth(data=df4, columns=['ZIP', 'PRICE'], geo_data=geofile, key_on='feature.properties.ZIP',
...
# --------------------------------------------------
# REALSTATE CATEGORY DISTRIBUTION
# --------------------------------------------------
